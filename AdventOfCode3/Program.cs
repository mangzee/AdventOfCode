...
namespace AdventOfCode3
{
    ...
    class Program
    {
        ...
        {
            var aPath = inputA.Split(",").Select(x => ConvertDirection(x)).ToList();
            var bPath = inputB.Split(",").Select(x => ConvertDirection(x)).ToList();
            var aPoints = TraceWire(aPath);
            var bPoints = TraceWire(bPath);

            var intersectingPoints = aPoints.Keys.Intersect(bPoints.Keys);

            return intersectingPoints.Min(i => Math.Abs(i.X) + Math.Abs(i.Y)).ToString();
        }
        ...
        public static string PartTwo()
        {
            ...
            return intersectingPoints.Min(i => aPoints[i] + bPoints[i]).ToString();
        }
        ...
        private static (Direction dir, int length) ConvertDirection(string input)
        {
            var dir = GetDirection(input[0]);
            var len = Convert.ToInt32(input.Substring(1));
            return (dir, len);
        }
